...
std::vector<TypePtr> DeltaSplitReader::adaptColumns(
  ...
  // Delta Lake stores all column data in data files
  for (const auto& childSpec : childrenSpecs) {
    ...
    if (auto iter = hiveSplit_->infoColumns.find(fieldName);
      ...
      if (outputTypeIdx.has_value() && fileTypeIdx.has_value()) {
        // Column exists in both file and output schema
        ...
        // Handle columns missing from the data file:
        // 1. Partition columns: Values stored in partition metadata
        // 2. Schema evolution: Column added after file was written
